...
public:
                               ...
                               const ufc::cell& c) const
  {
    ...
    const double G0 =  - K_10*W1*det;
    const double G1 =  - K_00*W1*det;
    const double G2 = K_11*W1*det;
    const double G3 = K_00*W1*det;
    const double G4 = W1*det*(K_00*K_10 + K_01*K_11);
    const double G5 =  - K_01*W1*det;
    const double G6 = W1*det*(K_00*K_00 + K_01*K_01);
    const double G7 =  - K_11*W1*det;
    const double G8 = W1*det*(K_10*K_10 + K_11*K_11);
    const double G9 = K_10*W1*det;
    const double G10 = K_01*W1*det;
    ...
    // Number of operations to compute element tensor for following IP loop = 177
    // Only 1 integration point, omitting IP loop.
    
    // Number of operations for primary indices: 49
    for (unsigned int j = 0; j < 7; j++)
    {
      for (unsigned int k = 0; k < 7; k++)
      {
        // Number of operations to compute entry: 1
        A[j*7 + k] += 0.000000000000000;
      }// end loop over 'k'
    }// end loop over 'j'
    ...
    for (unsigned int j = 0; j < 2; j++)
    {
      for (unsigned int k = 0; k < 2; k++)
      {
        ...
        A[nzc1[j]*7 + nzc2[k]] += FE1_C0_D01[0][j]*FE1_C0_D01[0][k]*G4;
        // Number of operations to compute entry: 3
        A[nzc2[j]*7 + nzc1[k]] += FE1_C0_D01[0][j]*FE1_C0_D01[0][k]*G4;
        // Number of operations to compute entry: 3
        A[nzc4[j]*7 + nzc5[k]] += FE1_C0_D01[0][j]*FE1_C0_D01[0][k]*G4;
        // Number of operations to compute entry: 3
        ...
        // Number of operations to compute entry: 3
        A[nzc5[j]*7 + nzc4[k]] += FE1_C0_D01[0][j]*FE1_C0_D01[0][k]*G4;
      }// end loop over 'k'
    }// end loop over 'j'
    ...
    for (unsigned int j = 0; j < 2; j++)
    {
      ...
      A[nzc1[j]*7 + 6] += FE1_C0_D01[0][j]*G0;
      // Number of operations to compute entry: 2
      A[nzc2[j]*7 + 6] += FE1_C0_D01[0][j]*G1;
      // Number of operations to compute entry: 2
      A[nzc5[j]*7 + 6] += FE1_C0_D01[0][j]*G5;
      // Number of operations to compute entry: 2
      A[nzc4[j]*7 + 6] += FE1_C0_D01[0][j]*G7;
    }// end loop over 'j'
    ...
    {
      // Number of operations to compute entry: 2
      A[6*7 + nzc4[k]] += FE1_C0_D01[0][k]*G2;
      // Number of operations to compute entry: 2
      A[6*7 + nzc2[k]] += FE1_C0_D01[0][k]*G3;
      // Number of operations to compute entry: 2
      A[6*7 + nzc1[k]] += FE1_C0_D01[0][k]*G9;
      // Number of operations to compute entry: 2
      A[6*7 + nzc5[k]] += FE1_C0_D01[0][k]*G10;
    }// end loop over 'k'
  }
