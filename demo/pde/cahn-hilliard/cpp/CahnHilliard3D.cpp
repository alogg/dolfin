                                    ...
                                    const ufc::cell& c) const
{
    ...
    double G[15];
    G[0] =  - det*w[1][0]*(K_10*K_20 + K_11*K_21 + K_12*K_22);
    G[1] = det*w[3][0]*w[4][0]*(K_10*K_20 + K_11*K_21 + K_12*K_22);
    G[2] = det*w[3][0]*w[4][0]*(K_00*K_20 + K_01*K_21 + K_02*K_22);
    G[3] =  - det*w[1][0]*(K_00*K_00 + K_01*K_01 + K_02*K_02);
    G[4] =  - det*w[1][0]*(K_00*K_10 + K_01*K_11 + K_02*K_12);
    G[5] = det*w[3][0]*w[4][0]*(K_10*K_10 + K_11*K_11 + K_12*K_12);
    G[6] =  - det*w[1][0]*(K_20*K_20 + K_21*K_21 + K_22*K_22);
    G[7] =  - det*w[1][0]*(K_00*K_20 + K_01*K_21 + K_02*K_22);
    G[8] = det*w[3][0]*w[4][0]*(K_00*K_00 + K_01*K_01 + K_02*K_02);
    G[9] = det*w[3][0]*w[4][0]*(K_00*K_10 + K_01*K_11 + K_02*K_12);
    G[10] =  - det*w[1][0]*(K_10*K_10 + K_11*K_11 + K_12*K_12);
    G[11] = det*w[3][0]*w[4][0]*(K_20*K_20 + K_21*K_21 + K_22*K_22);
    G[12] = 12.000000000000000*det*w[2][0];
    G[13] = -2.000000000000000*det*w[2][0];
    ...
    for (unsigned int ip = 0; ip < 27; ip++)
    {
      ...
      for (unsigned int j = 0; j < 2; j++)
      {
        for (unsigned int k = 0; k < 2; k++)
        {
          ...
          A[nzc6[j]*8 + nzc5[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[0];
          // Number of operations to compute entry: 3
          ...
          // Number of operations to compute entry: 3
          A[nzc7[j]*8 + nzc7[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[3];
          // Number of operations to compute entry: 3
          A[nzc7[j]*8 + nzc6[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[4];
          // Number of operations to compute entry: 3
          A[nzc2[j]*8 + nzc2[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[5];
          // Number of operations to compute entry: 3
          A[nzc5[j]*8 + nzc5[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[6];
          // Number of operations to compute entry: 3
          A[nzc5[j]*8 + nzc7[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[7];
          // Number of operations to compute entry: 3
          A[nzc3[j]*8 + nzc3[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[8];
          // Number of operations to compute entry: 3
          A[nzc5[j]*8 + nzc6[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[0];
          // Number of operations to compute entry: 3
          A[nzc7[j]*8 + nzc5[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[7];
          // Number of operations to compute entry: 3
          A[nzc3[j]*8 + nzc2[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[9];
          // Number of operations to compute entry: 3
          A[nzc6[j]*8 + nzc6[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[10];
          // Number of operations to compute entry: 3
          A[nzc6[j]*8 + nzc7[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[4];
          // Number of operations to compute entry: 3
          A[nzc1[j]*8 + nzc1[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[11];
          // Number of operations to compute entry: 3
          A[nzc2[j]*8 + nzc3[k]] += FE1_C0_D001[ip][j]*FE1_C0_D001[ip][k]*I[9];
          // Number of operations to compute entry: 3
      ...
      for (unsigned int j = 0; j < 4; j++)
      {
        ...
        {
          // Number of operations to compute entry: 3
          A[nzc4[j]*8 + nzc4[k]] += FE1_C0[ip][j]*FE1_C0[ip][k]*I[12];
          // Number of operations to compute entry: 3
          A[nzc4[j]*8 + nzc0[k]] += FE1_C0[ip][j]*FE1_C0[ip][k]*I[13];
          // Number of operations to compute entry: 3
          A[nzc0[j]*8 + nzc4[k]] += FE1_C0[ip][j]*FE1_C0[ip][k]*I[13];
        }// end loop over 'k'
      }// end loop over 'j'
    }// end loop over 'ip'
}
                                    ...
                                    const ufc::cell& c) const
{
    ...
    double G[21];
    G[0] = det*w[4][0]*w[5][0]*(K_00*K_20 + K_01*K_21 + K_02*K_22);
    G[1] = det*w[4][0]*(K_20*K_20 + K_21*K_21 + K_22*K_22 - w[5][0]*(K_20*K_20 + K_21*K_21 + K_22*K_22));
    G[2] = det*w[4][0]*w[5][0]*(K_20*K_20 + K_21*K_21 + K_22*K_22);
    G[3] = det*w[4][0]*w[5][0]*(K_10*K_20 + K_11*K_21 + K_12*K_22);
    G[4] = det*w[4][0]*(K_00*K_20 + K_01*K_21 + K_02*K_22 - w[5][0]*(K_00*K_20 + K_01*K_21 + K_02*K_22));
    G[5] = det*w[4][0]*(K_10*K_20 + K_11*K_21 + K_12*K_22 - w[5][0]*(K_10*K_20 + K_11*K_21 + K_12*K_22));
    ...
    G[9] = det*w[4][0]*w[5][0]*(K_00*K_10 + K_01*K_11 + K_02*K_12);
    G[10] = det*w[4][0]*w[5][0]*(K_10*K_10 + K_11*K_11 + K_12*K_12);
    G[11] = det*w[4][0]*(K_00*K_10 + K_01*K_11 + K_02*K_12 - w[5][0]*(K_00*K_10 + K_01*K_11 + K_02*K_12));
    G[12] = det*w[4][0]*(K_10*K_10 + K_11*K_11 + K_12*K_12 - w[5][0]*(K_10*K_10 + K_11*K_11 + K_12*K_12));
    ...
    G[16] = det*w[4][0]*w[5][0]*(K_00*K_00 + K_01*K_01 + K_02*K_02);
    G[17] = det*w[4][0]*(K_00*K_00 + K_01*K_01 + K_02*K_02 - w[5][0]*(K_00*K_00 + K_01*K_01 + K_02*K_02));
    G[18] = -2.000000000000000*det*w[3][0];
    G[19] = 6.000000000000000*det*w[3][0];
