...
public:
                               ...
                               const ufc::cell& c) const
  {
    ...
    const static double FE0_C1_D10[4][2] = \
    {{3, -3},
    {3, -3},
    {3, -3},
    {3, -3}};
    ...
    const static double FE0_C0_D10[4][6] = \
    {{2, -1, -2, 1, -1, 2},
    {2, -1, -2, 1, -1, 2},
    {2, -1, -2, 1, -1, 2},
    {2, -1, -2, 1, -1, 2}};
    ...
    const static double FE0_C0_D01[4][2] = \
    {{-3, 3},
    {-3, 3},
    {-3, 3},
    {-3, 3}};
    // Array of non-zero columns
    static const unsigned int nzc3[2] = {2, 3};
    ...
    // Number of operations to compute geometry constants: 73
    const double G0 = det*1/detJ*(J_00*Jinv_10 + J_10*Jinv_11);
    const double G1 =  - det*1/detJ*(J_00*Jinv_00 + J_10*Jinv_01);
    const double G2 =  - det*1/detJ*(J_01*Jinv_00 + J_11*Jinv_01);
    const double G3 = det*1/detJ*(J_01*Jinv_00 + J_11*Jinv_01);
    const double G4 =  - det*1/detJ*(J_00*Jinv_10 + J_10*Jinv_11);
    const double G5 = det*1/detJ*(J_00*Jinv_00 + J_10*Jinv_01);
    const double G6 = det*1/detJ*(J_01*Jinv_10 + J_11*Jinv_11);
    const double G7 = det*1/(detJ*detJ)*(J_01*J_01 + J_11*J_11);
    const double G8 = det*1/(detJ*detJ)*(J_00*J_01 + J_10*J_11);
    const double G9 = det*1/(detJ*detJ)*(J_00*J_00 + J_10*J_10);
    const double G10 =  - det*1/detJ*(J_01*Jinv_10 + J_11*Jinv_11);
    ...
    // Total number of operations to compute element tensor: 2101
    
    // Loop quadrature points for integral
    // Number of operations to compute element tensor for following IP loop = 2028
    for (unsigned int ip = 0; ip < 4; ip++)
    {
      ...
      for (unsigned int k = 0; k < 6; k++)
      {
        ...
        A[6*7 + nzc2[k]] += FE0_C0_D10[ip][k]*Gip5;
        // Number of operations to compute entry = 2
        A[6*7 + nzc0[k]] += FE0_C1_D01[ip][k]*Gip6;
      }// end loop over 'k'
      ...
      for (unsigned int j = 0; j < 6; j++)
      {
        for (unsigned int k = 0; k < 6; k++)
        {
          ...
          A[nzc5[j]*7 + nzc5[k]] += FE0_C1[ip][j]*FE0_C1[ip][k]*Gip7;
          // Number of operations to compute entry = 3
          A[nzc5[j]*7 + nzc6[k]] += FE0_C0[ip][k]*FE0_C1[ip][j]*Gip8;
          // Number of operations to compute entry = 3
          A[nzc6[j]*7 + nzc6[k]] += FE0_C0[ip][j]*FE0_C0[ip][k]*Gip9;
          // Number of operations to compute entry = 3
          A[nzc6[j]*7 + nzc5[k]] += FE0_C0[ip][j]*FE0_C1[ip][k]*Gip8;
        }// end loop over 'k'
      }// end loop over 'j'
      ...
      {
        // Number of operations to compute entry = 2
        A[nzc1[j]*7 + 6] += FE0_C1_D10[ip][j]*Gip2;
        // Number of operations to compute entry = 2
        A[nzc3[j]*7 + 6] += FE0_C0_D01[ip][j]*Gip4;
      }// end loop over 'j'
      ...
      {
        // Number of operations to compute entry = 2
        A[6*7 + nzc3[k]] += FE0_C0_D01[ip][k]*Gip0;
        // Number of operations to compute entry = 2
        A[6*7 + nzc1[k]] += FE0_C1_D10[ip][k]*Gip3;
      }// end loop over 'k'
      ...
      {
        // Number of operations to compute entry = 2
        A[nzc2[j]*7 + 6] += FE0_C0_D10[ip][j]*Gip1;
        // Number of operations to compute entry = 2
        A[nzc0[j]*7 + 6] += FE0_C1_D01[ip][j]*Gip10;
      }// end loop over 'j'
    }// end loop over 'ip'
  }
...
public:
  ...
  virtual const char* signature() const
  {
    return "Form([Integral(Sum(Product(IndexSum(Indexed(ListTensor(Indexed(SpatialDerivative(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 1), MultiIndex((Index(0),), {Index(0): 2})), MultiIndex((FixedIndex(0),), {})), Indexed(SpatialDerivative(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 1), MultiIndex((Index(0),), {Index(0): 2})), MultiIndex((FixedIndex(1),), {}))), MultiIndex((Index(0),), {Index(0): 2})), MultiIndex((Index(0),), {Index(0): 2})), Indexed(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 0), MultiIndex((FixedIndex(2),), {FixedIndex(2): 3}))), Sum(IndexSum(Product(Indexed(ListTensor(Indexed(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 0), MultiIndex((FixedIndex(0),), {FixedIndex(0): 3})), Indexed(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 0), MultiIndex((FixedIndex(1),), {FixedIndex(1): 3}))), MultiIndex((Index(1),), {Index(1): 2})), Indexed(ListTensor(Indexed(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 1), MultiIndex((FixedIndex(0),), {FixedIndex(0): 3})), Indexed(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 1), MultiIndex((FixedIndex(1),), {FixedIndex(1): 3}))), MultiIndex((Index(1),), {Index(1): 2}))), MultiIndex((Index(1),), {Index(1): 2})), Product(IntValue(-1, (), (), {}), Product(IndexSum(Indexed(ListTensor(Indexed(SpatialDerivative(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 0), MultiIndex((Index(2),), {Index(2): 2})), MultiIndex((FixedIndex(0),), {})), Indexed(SpatialDerivative(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 0), MultiIndex((Index(2),), {Index(2): 2})), MultiIndex((FixedIndex(1),), {}))), MultiIndex((Index(2),), {Index(2): 2})), MultiIndex((Index(2),), {Index(2): 2})), Indexed(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 1), MultiIndex((FixedIndex(2),), {FixedIndex(2): 3})))))), Measure('cell', 0, None))])";
  }
...
public:
                               ...
                               const ufc::cell& c) const
  {
    ...
    const double G0 = W1*det*w[0][0];
    
    // Compute element tensor using UFL quadrature representation
    // Optimisations: ('simplify expressions', True), ('ignore zero tables', True), ('non zero columns', True), ('remove zero terms', True), ('ignore ones', True)
    // Total number of operations to compute element tensor: 3
...
public:
  ...
  virtual const char* signature() const
  {
    return "Form([Integral(Product(Function(FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0), 0), Indexed(BasisFunction(MixedElement(*[FiniteElement('Brezzi-Douglas-Marini', Cell('triangle', 1), 1), FiniteElement('Discontinuous Lagrange', Cell('triangle', 1), 0)], **{'value_shape': (3,) }), 0), MultiIndex((FixedIndex(2),), {FixedIndex(2): 3}))), Measure('cell', 0, None))])";
  }
...
public:
  ...
  CoefficientSpace_f(const dolfin::Mesh & mesh):
      dolfin::FunctionSpace(dolfin::reference_to_no_delete_pointer(mesh),
                            ...
                            boost::shared_ptr<const dolfin::DofMap>(new dolfin::DofMap(boost::shared_ptr<ufc::dof_map>(new mixedpoisson_1_dof_map_1()), dolfin::reference_to_no_delete_pointer(mesh))))
  {
    // Do nothing
  }
...
public:
  ...
  Form_0_FunctionSpace_0(const dolfin::Mesh & mesh):
      dolfin::FunctionSpace(dolfin::reference_to_no_delete_pointer(mesh),
                            ...
                            boost::shared_ptr<const dolfin::DofMap>(new dolfin::DofMap(boost::shared_ptr<ufc::dof_map>(new mixedpoisson_0_dof_map_0()), dolfin::reference_to_no_delete_pointer(mesh))))
  {
    // Do nothing
  }
...
public:
  ...
  Form_0_FunctionSpace_1(const dolfin::Mesh & mesh):
      dolfin::FunctionSpace(dolfin::reference_to_no_delete_pointer(mesh),
                            ...
                            boost::shared_ptr<const dolfin::DofMap>(new dolfin::DofMap(boost::shared_ptr<ufc::dof_map>(new mixedpoisson_0_dof_map_1()), dolfin::reference_to_no_delete_pointer(mesh))))
  {
    // Do nothing
  }
...
public:
  ...
  Form_1_FunctionSpace_0(const dolfin::Mesh & mesh):
      dolfin::FunctionSpace(dolfin::reference_to_no_delete_pointer(mesh),
                            ...
                            boost::shared_ptr<const dolfin::DofMap>(new dolfin::DofMap(boost::shared_ptr<ufc::dof_map>(new mixedpoisson_1_dof_map_0()), dolfin::reference_to_no_delete_pointer(mesh))))
  {
    // Do nothing
  }
