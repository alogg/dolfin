...
_Mesh_. Other entities must be explicitly created by calling init().
For example, all edges in a mesh may be created by a call to
mesh.init(1). Similarly, connectivities such as all edges connected to
a given vertex must also be explicitly created (in this case by a call
to mesh.init(0, 1)).
...
Arguments* mesh A _Mesh_ object. ";

%feature("docstring")  dolfin::Mesh::Mesh "

Create mesh from data file.

Arguments* filename A string, name of file to load. ";
...
>>> mesh = dolfin.UnitSquare(2,2) >>> mesh.num_vertices() 9 ";

%feature("docstring")  dolfin::Mesh::num_edges "

Get number of edges in mesh.

Returns* integer Number of edges.

Example* .. warning::

Not C++ syntax.

>>> mesh = dolfin.UnitSquare(2,2) >>> mesh.num_edges() 0 >>>
mesh.init(1) 16 >>> mesh.num_edges() 16 ";

%feature("docstring")  dolfin::Mesh::num_faces "

Get number of faces in mesh.

Returns* integer Number of faces.

Example* .. warning::

Not C++ syntax.

>>> mesh = dolfin.UnitSquare(2,2) >>> mesh.num_faces() 8 ";

%feature("docstring")  dolfin::Mesh::num_facets "

Get number of facets in mesh.

Returns* integer Number of facets.

Example* .. warning::

Not C++ syntax.

>>> mesh = dolfin.UnitSquare(2,2) >>> mesh.num_facets() 0 >>>
mesh.init(0,1) >>> mesh.num_facets() 16 ";

%feature("docstring")  dolfin::Mesh::num_cells "

Get number of cells in mesh.

Returns* integer Number of cells.

Example* .. warning::

Not C++ syntax.

>>> mesh = dolfin.UnitSquare(2,2) >>> mesh.num_cells() 8 ";

%feature("docstring")  dolfin::Mesh::num_entities "

Get number of entities of given topological dimension.

Arguments* d An integer, topological dimension.

Returns* integer Number of entities of topological dimension d.

Example* .. warning::

Not C++ syntax.

>>> mesh = dolfin.UnitSquare(2,2) >>> mesh.init(0,1) >>>
mesh.num_entities(0) 9 >>> mesh.num_entities(1) 16 >>>
mesh.num_entities(2) 8 ";

%feature("docstring")  dolfin::Mesh::coordinates "

Get vertex coordinates.

Returns* An array of doubles Coordinates of all vertices.

Example* .. warning::

Not C++ syntax.

>>> mesh = dolfin.UnitSquare(1,1) >>> mesh.coordinates() array([[ 0.,
0.], [ 1., 0.], [ 0., 1.], [ 1., 1.]]) ";
...
Returns* An array of integers Connectivity for all cells.

Example* .. warning::

Not C++ syntax.

>>> mesh = dolfin.UnitSquare(1,1) >>> mesh.coordinates() array([[0, 1,
3], [0, 2, 3]]) ";
...
Arguments* dim An integer, topological dimension.
...
Returns* _MeshTopology_ The topology object associated with the mesh.
";
...
Returns* _MeshGeometry_ The geometry object associated with the mesh.
";
...
Returns* _IntersectionOperator_ The intersection operator object
...
Returns* _MeshData_ The mesh data object associated with the mesh. ";
...
Returns* _CellType_ The cell type object associated with the mesh. ";
...
Returns* integer Number of created entities. ";

%feature("docstring")  dolfin::Mesh::init "

Compute connectivity between given pair of dimensions.

Arguments* d0 An integer, topological dimension.

d1 An integer, topological dimension. ";
...
Returns* bool Return true iff topology is ordered according to the UFC
numbering. ";
...
Arguments* boundary A _BoundaryMesh_ object.

method A ALEType (enum). Method which defines how the coordinates
should be moved, default is *hermite*. ";
...
Arguments* function A _Function_ object. ";

%feature("docstring")  dolfin::Mesh::smooth "

Smooth internal vertices of mesh by local averaging.

Arguments* num_iterations An integer, number of iterations to perform
...
harmonic_smoothing A bool, flag to turn on harmonics smoothing,
...
Arguments* sub_domain A _SubDomain_ object.
...
Arguments* point A _Point_ object.

ids_result A set of integers. The cell ids which are intersected are
stored in a set for efficiency reasons, to avoid to sort out
duplicates later on. ";
...
Arguments* points A vector of _Point_ objects.
...
Arguments* entity A _MeshEntity_ object.

ids_result A list of integers. The ids of the intersected cells are
saved in a list. This is more efficent than using a set and allows a
map between the (external) cell and the intersected cell of the mesh.
";
...
Arguments* entities A vector of _MeshEntity_ objects.
...
Arguments* another_mesh A _Mesh_ object.
...
Returns* integer The first id of the cell, which contains the point,
returns -1 if no cell is intersected. ";
...
Returns* Point The point inside the mesh which is closest to the
point. ";
...
Returns* integer The index of the cell in the mesh which is closest to
point.

Example* .. code-block:: c++

UnitSquare mesh(1, 1); Point point(0.0, 2.0); info(\"%d\",
...
Returns* pair <_Point_, integer> The point inside the mesh and the
corresponding cell index which is closest to the point query. ";

%feature("docstring")  dolfin::Mesh::hmin "

Compute minimum cell diameter.

Returns* double The minimum cell diameter, the diameter is computed as
...
>>> mesh = dolfin.UnitSquare(2,2) >>> mesh.hmin() 0.70710678118654757
";
...
Returns* double The maximum cell diameter, the diameter is computed as
two times the circumradius (http://mathworld.wolfram.com).
...
>>> mesh = dolfin.UnitSquare(2,2) >>> mesh.hmax() 0.70710678118654757
";
...
Arguments* verbose A bool, flag to turn on additional output.

Returns* string An informal representation of the mesh.

Example* .. warning::

Not C++ syntax.

>>> mesh = dolfin.UnitSquare(2,2) >>> mesh.str(False) '< Mesh of
topological dimension 2 (triangles) with 9 vertices and 8 cells,
ordered>' ";
